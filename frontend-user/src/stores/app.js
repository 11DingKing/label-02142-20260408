...
export const useAppStore = defineStore("app", () => {
  ...
  const currentMotto = computed(
    () => mottos.value[currentMottoIndex.value] || "",
  );
  ...
  function nextMotto() {
    currentMottoIndex.value =
      (currentMottoIndex.value + 1) % mottos.value.length;
  }
  ...
  function startTimers() {
    ...
    if (config.mottos.enabled) {
      ...
      logger.debug("话术切换定时器已启动", {
        interval: config.mottos.switchInterval,
      });
    }

    // 背景图片刷新
    setInterval(loadBackgroundImage, config.background.refreshInterval);
    logger.debug("背景刷新定时器已启动", {
      interval: config.background.refreshInterval,
    });

    // 每日一言刷新
    if (config.dailyQuote.enabled) {
      setInterval(loadDailyQuote, config.dailyQuote.refreshInterval);
      logger.debug("一言刷新定时器已启动", {
        interval: config.dailyQuote.refreshInterval,
      });
    }
    ...
    if (config.serverInfo.enabled) {
      ...
      logger.debug("服务器信息刷新定时器已启动", {
        interval: config.serverInfo.refreshInterval,
      });
    }
